"""
Experimento 04: Motor de Full State e Micro-Latência (A Cereja do Bolo SOTA)
-------------------------------------------------------------------------
Meu Objetivo: Otimização implacável do Threshold (DRE/ROI) consolidando os 
buffer in-memory (Client + Merchant) e anexando minha heurística mais destrutiva: A Micro-Latência.

Premissas e Visão de Engenharia:
1. Tracking Mútuo: Padrões acelerados nos dados da maquininha denunciam fraude ativa no lado do Lojista.
2. Bloqueio de Testadores Bin: Extrair os milissegundos cravados transcorridos desde o último 
   evento flagra estritamente bots de força bruta financeira ("time_since_last_trans").
"""
